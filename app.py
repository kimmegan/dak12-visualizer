...
import io as _ioimport streamlit as st
import pandas as pd
import numpy as np
from scipy.interpolate import interp1d
import plotly.graph_objects as go
...
# If averaging is ON, calculate mean and SD across all selected sheets
global_stats = None
if show_average_and_bands and selected_keys:
    ...
    if dfs:
        # Assume all sheets have same frequencies
...
if show_average_and_bands and global_stats:
    ...
    # Use a neutral color that represents the averaged data
    ...
    # Frequency markers (use averaged data)
    if marker_freqs:
    ...
    # Show individual sheets (not averaged)
...
def build_export_png(selected_keys, st_session, legend_names, display_names,
    ...
    # If averaging is ON, show mean + error bands
    if show_average_and_bands and global_stats:
        stats = global_stats
        mean_color = "#1f77b4"
        sigma_color = "#d62728"
